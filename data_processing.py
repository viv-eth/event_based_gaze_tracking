...
# make a 3D plot of the frame pngs 
# where the image spans on the x and y axis
# and the frames are stacked on the z axis
# fig = plt.figure()
# ax = fig.add_subplot(111, projection='3d')

# for frame_path, frame_timestamp in zip(frame_paths, frame_timestamps):
#     # plot the first frame in 3D where the image spans on the x and y axis
#     # and the z axis is the timestamp
#     # read the image
#     img = cv2.imread(frame_path)
#     # get the height and width
#     height, width, channels = img.shape
#     # get the x and y coordinates but flip along the y axis
#     x = np.linspace(0, width, width)
#     y = np.linspace(0, height, height)
#     y = np.flip(y)
#     x, y = np.meshgrid(x, y)
#     # get the z coordinates
#     z = np.ones((height, width)) * frame_timestamp
#     ax.plot_surface(x, y, z, rstride=1, cstride=1, facecolors=img/255, shade=False, alpha=0.5)

# ax.set_xlabel('X')
# ax.set_ylabel('Y')
# ax.set_zlabel('Timestamp')
# # save the 3D plot in an interactive format
...
#     # Read the image
#     img = cv2.imread(frame_path)
#     # rescale the image to FRAME_WIDTH x FRAME_HEIGHT
...
#     # Get the z coordinates
#     z = np.ones((height, width)) * frame_timestamp
#     # z = np.ones_like(x) * frame_timestamp

#     texture = (255 * img).astype(np.uint8)
#     intensity = img.flatten()

#     # Add a surface trace to the figure
...
# plot all high density events with the pupil centers
for high_density_event in high_density_buffers:
    # get the event dataframe
    high_density_event_df = event_df[event_df['frame_number'].isin(high_density_event)]

    unique_frame_numbers = high_density_event_df['frame_number'].unique()
    frame_paths = frame_df[frame_df['frame_number'].isin(unique_frame_numbers)]['img_path'].values
    frame_timestamps = frame_df[frame_df['frame_number'].isin(unique_frame_numbers)]['timestamp'].values
    pupil_timestamps = frame_df[frame_df['frame_number'].isin(unique_frame_numbers)]['timestamp'].values
    pupil_centers = frame_df[frame_df['frame_number'].isin(unique_frame_numbers)]['pupil'].values
    ...
    # get the x and y coordinates
    x = high_density_event_df['x'].values
    y = high_density_event_df['y'].values
    z = high_density_event_df['timestamp'].values

    # get the polarity
    polarity = high_density_event_df['polarity'].values
